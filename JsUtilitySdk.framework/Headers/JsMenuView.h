...
@class Prefix(MenuView);
@protocol Prefix(MenuViewDelegate) <NSObject>

@required
- (NSArray <Prefix(MenuItemVm) *>*)jsMenuViewSetDatasource:(Prefix(MenuView) *)view;

@optional
//组件的时间,需要把组件的序号和组件vm传递出去.
- (void)jsMenuView:(JsMenuView *)view clickMenuAtIndex:(int)index vm:(Prefix(MenuItemVm) *)vm;
...
@interface Prefix(MenuView) : UIView

@property (nonatomic,assign)id<Prefix(MenuViewDelegate)> delegate;
